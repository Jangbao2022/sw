...
create table organization(
                           ...
                           honor VARCHAR(1000) NOT NULL COMMENT '荣誉',
                           `show` VARCHAR(1000) NOT NULL COMMENT '展示图',
...
create table superior(
                       ...
                       country VARCHAR(30) NOT NULL COMMENT '国家',
                       interal INT(1) NOT NULL COMMENT '国内/外',
                       honor VARCHAR(1000)  COMMENT '荣誉',
                       `show` VARCHAR(1000) 	NOT NULL COMMENT '图片展示',
                       `describe` VARCHAR(1000) NOT NULL COMMENT '简介',
                       gmt_created datetime,
                       gmt_modified datetime
)DEFAULT CHARSET='utf8';
...
create table  video_class(
                           id INT8 PRIMARY KEY AUTO_INCREMENT,
                           `name` VARCHAR(100) NOT NULL COMMENT '动作名',
                           `describe` VARCHAR(100) NOT NULL COMMENT '动作简介',
                           `show` VARCHAR(1000) NOT NULL COMMENT '动作展示图',
                           gmt_created datetime,
                           gmt_modified datetime
)DEFAULT CHARSET='utf8';
...
create table course(
                     ...
                     `name` VARCHAR(100) NOT NULL  COMMENT '教程名',
                     link VARCHAR(100) NOT NULL COMMENT '链接',
                     `describe` VARCHAR(1000) NOT NULL COMMENT'简介',
                     `show` VARCHAR(1000) NOT NULL COMMENT'展示',
                     gmt_created datetime,
                     gmt_modified datetime,
                     FOREIGN KEY(o_id) REFERENCES organization(id),
                     FOREIGN KEY(s_id) REFERENCES superior(id),
...
create table shop_class(
                         id INT8 PRIMARY KEY AUTO_INCREMENT,
                         `name` VARCHAR(100) NOT NULL COMMENT'类名',
                         `describe` VARCHAR(1000) NOT NULL COMMENT '简介',
                         `show` VARCHAR(1000) NOT NULL COMMENT '图片展示',
                         gmt_created datetime,
                         gmt_modified datetime
...
create table goods(
  ...
  `get` VARCHAR(1000) NOT NULL COMMENT '如何购买',
  `show` VARCHAR(1000) NOT NULL COMMENT '展示图',
  gmt_created datetime,
  gmt_modified datetime,
...
create table `comments` (
                          ...
                          `comment` VARCHAR(1000) NOT NULL COMMENT '评价',
                          `after_comment` VARCHAR(1000) COMMENT '追评',
                          gmt_created datetime,
                          gmt_modified datetime,
